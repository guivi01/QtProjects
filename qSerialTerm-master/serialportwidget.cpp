...
//QT_USE_NAMESPACE_QSERIALPORT

//#define SerialPortInfo QSerialPortInfo
//#define SerialPort QSerialPort
/*
...
void SerialPortWidget::on_portComboBox_currentIndexChanged(int index)
{
  if (index == -1) {
    ...
    QList<qint32> baudRateList = serialPortInfoList[index].standardBaudRates();// standardRates();
...
void SerialPortWidget::on_dataBitsComboBox_currentIndexChanged(int index)
{
  if (index != -1 && serialPort != 0) {
    switch(index){
    case 0:
        if(serialPort->setDataBits( QSerialPort::Data5)) {
            validateCommunicationSettings();
            return;
        } else {
        QMessageBox::warning(this,
        ...
        break;
    case 1:
        if(serialPort->setDataBits( QSerialPort::Data6)) {
            validateCommunicationSettings();
            return;
        } else {
        QMessageBox::warning(this,
